...
Widget::Widget (QWidget* parent) : QWidget (parent)
{
    ...
    connect (downloader, &Downloader::onReady, xmlparser, [=] () {
        ...
        // qDebug () << main_TAF->Title->request_index () << "\t" << main_TAF->vec_taf.at (0).station_id ();
        //  формируется строка прогноза на Label
        weather->set_name_airport (downloader->get_name_airport ());
        weather->set_text_forecast ("\n" + main_TAF->Title->request_index () + "\n" + main_TAF->vec_taf.at (0).station_id ());
        // показываем Label
        weather->show ();
