...
void XMLParser::set_dir_file_with_xml(const QString &dir) {
  this->_dir_file_with_xml = dir;
}
  ...
  while (!reader.atEnd() && !reader.hasError()) {
    ...
    if (token == QXmlStreamReader::StartElement) {
      if (reader.name().toString() == "response") {
        ...
        _main_taf->Title->set_num_results(
