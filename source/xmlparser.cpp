...
void XMLParser::set_vec_buf_xml(const std::vector<std::tuple<QString, QString>>
                                    &in_vec_tuple_data_for_parsing) {
    ...
    while (!xmlReader.atEnd() && !xmlReader.hasError()) {
      ...
      if (token == QXmlStreamReader::StartElement) {
        if (xml_name == "response"                         //
          ...
          if (sign_buff == "taf") {
            taf->set_raw_text(xmlReader.readElementText());
          };
          if (sign_buff == "metar") {
          ...
          if (sign_buff == "taf") {
            taf->set_station_id(xmlReader.readElementText());
          };
          if (sign_buff == "metar") {
            metar->setStation_id(xmlReader.readElementText());
          };
        } else if (xml_name == "observation_time") {
          ...
          taf->set_valid_time_to(xmlReader.readElementText());
          ...
          forecast->set_condition(
              ...
              std::make_tuple(xmlReader.attributes()
                                  .value("turbulence_intensity")
                                  .toString(),
                              xmlReader.attributes()
                                  .value("turbulence_min_alt_ft_agl")
                                  .toString(),
                              xmlReader.attributes()
          ...
          forecast->set_condition(
              "icing_condition",
              std::make_tuple(
                  xmlReader.attributes().value("icing_intensity").toString(),
                  xmlReader.attributes()
                      .value("icing_min_alt_ft_agl")
                      .toString(),
                  xmlReader.attributes()
                      .value("icing_max_alt_ft_agl")
                      .toString()));
        } else
      ...
      if (token == QXmlStreamReader::EndElement && xml_name == "forecast") {
        taf->v_forecasts.emplace_back(std::move(forecast));
      }
